...
def load_frames(
    ...
    if screen_ids:
        ...
        frames.extend(
            Frame(id=r["id"], source="capture", text=r["text"] or "",
    ...
    if audio_ids:
        ...
        rows = conn.execute(
            ...
            f"FROM audio_frames WHERE id IN ({ph}) ORDER BY timestamp",
            audio_ids,
        ).fetchall()
        frames.extend(
            Frame(id=r["id"], source="audio", text=r["text"] or "",
    ...
    if os_event_ids:
        ...
        frames.extend(
            Frame(id=r["id"], source="os_event", text=r["data"] or "",
